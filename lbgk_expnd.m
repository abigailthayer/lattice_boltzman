...
% These are the weights for equilibrium distribution (smaller number =
% higher weight)
t1=4/9;   % Rest particle (ie. 1/9 makes particles stay in the same place)
t2=1/9;   % "Straight" directions (ie. 8/9 keeps particles from moving in straight directions)
t3=1/36;  % Diagonal directions (ie. 35/36 keeps particles from moving in diagonal directions)
...
F=repmat(density/9,[nx ny 9]); %fills matrix of [nx ny 9] dimensions with [density/9]
FEQ=F; %saves newly created matrix as the equilibrium distribution
...
load boundary.txt %impose boundary conditions 
BOUND = transpose(boundary); 
